...
static int _olua_fun_cclua_plugin_apple___olua_move(lua_State *L)
{
    olua_startinvoke(L);

    auto self = (cclua::plugin::apple *)olua_toobj(L, 1, "cclua.plugin.apple");
    olua_push_object(L, self, "cclua.plugin.apple");
...
static int _olua_cls_cclua_plugin_apple(lua_State *L)
{
    ...
    oluacls_func(L, "__olua_move", _olua_fun_cclua_plugin_apple___olua_move);
...
{
    olua_require(L, "apple",  luaopen_apple);
