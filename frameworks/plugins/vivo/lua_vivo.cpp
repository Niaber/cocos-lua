...
static int _olua_fun_cclua_plugin_vivo___olua_move(lua_State *L)
{
    olua_startinvoke(L);

    auto self = (cclua::plugin::vivo *)olua_toobj(L, 1, "cclua.plugin.vivo");
    olua_push_object(L, self, "cclua.plugin.vivo");
...
static int _olua_cls_cclua_plugin_vivo(lua_State *L)
{
    ...
    oluacls_func(L, "__olua_move", _olua_fun_cclua_plugin_vivo___olua_move);
...
{
    olua_require(L, "vivo",  luaopen_vivo);
